...
from canny_filter_method import applyCannyFilter2
...
from customRansac import customFindHomographyRectangles
import time
import json
...
rectangles =[]
if filter_flag:
    descriptors_scene, keypoints_scene, rectangles = applyCannyFilter2(
        img_scene, depth_image, sift_mask, keypoints_scene, descriptors_scene)
        ##filtering for rectangles
...
for rect in rectangles:
    ...
    polygon = Polygon([Point(rect[0]),Point(rect[1]),Point(rect[2])])
    points_to_use = []
    for i in range(len(good_matches)):
        # Get the keypoints from the good matches
        obj[i, 0] = keypoints_obj[good_matches[i].queryIdx].pt[0] #x
        obj[i, 1] = keypoints_obj[good_matches[i].queryIdx].pt[1] #y
        scene[i, 0] = keypoints_scene[good_matches[i].trainIdx].pt[0]
        scene[i, 1] = keypoints_scene[good_matches[i].trainIdx].pt[1]
        if polygon.contains(Point(int(scene[i, 0]), int(scene[i, 1]))):
            points_to_use.append([ obj[i, 0],obj[i, 1],scene[i, 0],scene[i, 1]])
    print("Iteration "+str(j))
    #print(len(points_to_use))
    thresh = 6
    if len(points_to_use)>thresh:
        print("Find homography")
        H, mask = customFindHomographyRectangles(obj, scene, thresh=thresh, correspondenceList2=points_to_use)  # migliore è 0.6 o 1.0
...
cv.waitKey()
cv.destroyAllWindows()
cv.imwrite("output_hough.jpg", img_matches)
